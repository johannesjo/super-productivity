...
export interface PersistenceBaseModel<T> {
  ...
  saveState(state: T, isDataImport?: boolean, isNoSyncChange?: boolean): Promise<unknown>;
}
...
export interface PersistenceForProjectModel<S, M> {
  ...
  save(projectId: string, state: S, isDataImport?: boolean, isNoSyncChange?: boolean): Promise<unknown>;
