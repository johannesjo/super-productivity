...
describe('AddTaskBarActionsComponent', () => {
  ...
  // Locales default to the mutable backing vars above so a test can flip them
  // (the ISO 8601 case); pass an override to pin one facade to a fixed locale.
  const mockAddTaskBarDataFacade = (localeOverride?: string): unknown => ({
    ...
    currentLocale: () => localeOverride ?? mockCurrentLocale,
    textLocale: () => localeOverride ?? mockTextLocale,
    formatTime: (timestamp: number) =>
      new Date(timestamp).toLocaleTimeString(localeOverride ?? mockCurrentLocale, {
        hour: 'numeric',
        minute: 'numeric',
      }),
