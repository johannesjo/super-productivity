...
// Runs the pending first change detection so `ngAfterViewInit`'s auto-focus
// fires and is discarded, leaving the spy to record only what the submit path
// does. The submit tests wait on a macrotask, which is long enough for that
// hook to land in the middle of the assertion otherwise.
const settleInitialAutoFocus = (
  fixture: ComponentFixture<AddTaskBarComponent>,
...
// Drains the microtask queue. The submit path resolves through the data
// facade, so it needs more than a single `await Promise.resolve()` tick.
...
describe('AddTaskBarComponent', () => {
  ...
  describe('onSubmitBtnClick', () => {
    it('should add the task and refocus the input for rapid entry', async () => {
      ...
      settleInitialAutoFocus(fixture, focusSpy);
      component.stateService.updateInputTxt('Buy milk');
      component.stateService.updateCleanText('Buy milk');

      component.onSubmitBtnClick();
      // Wait for the addTask promise (and its .finally) to settle. A macrotask
      // rather than a single microtask: the submit runs through the data facade,
      // which awaits before it resolves.
