...
import { selectWeekPlannerState } from '../store/week-planner.selectors';
...
export class AddTaskPanelComponent {
  ...
  filteredTasks$: Observable<TaskCopy[]> = combineLatest([
    ...
    this._store.select(selectWeekPlannerState),
  ]).pipe(
    withLatestFrom(
      this.allTasks$,
    ...
    map(([[value, weekPlannerState], tasks, tagFeatureState, projectFeatureState]) => {
      const lcv = value.toLowerCase();

      const allAddedIds = Object.keys(weekPlannerState.days)
        .filter((day) => day !== ADD_TASK_PANEL_ID)
        .map((day) => weekPlannerState.days[day])
        .flat();
