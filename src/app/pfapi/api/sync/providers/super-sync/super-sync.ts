...
import { WebdavApi } from '../webdav/webdav-api';
...
export class SuperSyncProvider
  implements SyncProviderServiceInterface<SyncProviderId.SuperSync>
{
  private static readonly L = 'SuperSyncProvider';
  ...
  // Composition: use WebdavApi internally
  ...
  async uploadFile(
    ...
    SyncLog.debug(SuperSyncProvider.L, 'uploadFile', {
      targetPath,
      localRev,
      isForceOverwrite,
    });
  ...
  async downloadFile(
    ...
    SyncLog.debug(SuperSyncProvider.L, 'downloadFile', { targetPath });
    const { filePath } = await this._getConfigAndPath(targetPath);
    ...
    return {
      rev: result.rev,
      legacyRev: result.legacyRev,
      dataStr: result.dataStr,
    };
  }

  async removeFile(targetPath: string): Promise<void> {
    SyncLog.debug(SuperSyncProvider.L, 'removeFile', { targetPath });
    const { filePath } = await this._getConfigAndPath(targetPath);
