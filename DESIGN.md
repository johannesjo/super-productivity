...
Reference screenshots for the surfaces above live in `apps/client/tests/visual/` and are
regenerated by the `e2e/baselines.e2e.ts` Playwright spec:

- `today-task-list.png` — Today workspace with a selected task + inspector
- `planner-week.png`, `schedule.png`, `boards.png`, `eisenhower.png`
- `notes.png`, `history.png`, `insights.png`, `focus.png`
- `command-palette.png`, `settings-appearance.png`

Regenerate with a dev server from `apps/client`:

```sh
bunx playwright test e2e/baselines.e2e.ts --config=<dev config>
```

Commit the resulting PNGs as the reference; diff them in review to catch density/layout
regressions on each surface.
