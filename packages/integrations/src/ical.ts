...
interface RawComponent {
  name: string;
  properties: Array<{ name: string; value: string; params: Record<string, string> }>;
}
...
const parseDateValue = (value: string): { iso: string; allDay: boolean } => {
  ...
  if (/^\d{8}T\d{6}Z$/.test(stripped)) {
    return {
      iso: new Date(
        Date.UTC(
          Number(stripped.slice(0, 4)),
          Number(stripped.slice(4, 6)) - 1,
          Number(stripped.slice(6, 8)),
          Number(stripped.slice(9, 11)),
          Number(stripped.slice(11, 13)),
        ),
      ).toISOString(),
      allDay: false,
    };
...
export const parseIcs = (text: string): CalendarEvent[] => {
  const components: RawComponent[] = [];
  let current: RawComponent | undefined;
  ...
  for (const component of components) {
    ...
    const prop = (name: string) =>
      component.properties.find((entry) => String(entry.name).toUpperCase() === name)
        ?.value ?? '';
