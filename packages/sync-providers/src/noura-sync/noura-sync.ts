...
import type { NouraSyncResponseValidators } from './response-validators';
import type { NouraSyncStorage } from './storage';
import {
  PROVIDER_ID_NOURA_SYNC,
  type NouraSyncPrivateCfg,
  type NouraSyncWebSocketAccess,
} from './noura-sync.model';
 ...
 */
class NouraSyncHttpStatusError extends Error {
  override readonly name = 'NouraSyncHttpStatusError';
...
export interface NouraSyncDeps {
  logger: SyncLogger;
  ...
  credentialStore: SyncCredentialStorePort<
    typeof PROVIDER_ID_NOURA_SYNC,
    NouraSyncPrivateCfg
  >;
  storage: NouraSyncStorage;
  responseValidators: NouraSyncResponseValidators;
  /**
   * Host-supplied fallback used whenever `NouraSyncPrivateCfg.baseUrl`
   ...
   * `NOURA_SYNC_DEFAULT_BASE_URL`; others pass their own URL.
   */
...
/**
 * NouraSync provider — operation-based sync.
 *
 ...
 */
export class NouraSyncProvider
  implements
    SyncProviderBase<typeof PROVIDER_ID_NOURA_SYNC, NouraSyncPrivateCfg>,
    OperationSyncCapable<'nouraSyncOps'>,
    RestoreCapable,
    NouraSyncWebSocketAccess
{
  readonly id = PROVIDER_ID_NOURA_SYNC;
  readonly isUploadForcePossible = false;
  readonly maxConcurrentRequests = 10;
  readonly supportsOperationSync = true;
  readonly providerMode = 'nouraSyncOps' as const;
  // NouraSync is E2EE-mandatory: the upload path must never push plaintext ops.
  ...
  constructor(private readonly _deps: NouraSyncDeps) {
    this.privateCfg = _deps.credentialStore;
  }

  private get _logLabel(): string {
    return 'NouraSyncProvider';
  }
   ...
   */
  private _isEncryptionHalfConfigured(cfg: NouraSyncPrivateCfg): boolean {
    return !!cfg.isEncryptionEnabled && !cfg.encryptKey;
  }

  async setPrivateCfg(cfg: NouraSyncPrivateCfg): Promise<void> {
  ...
  async downloadOps(
    ...
    limit?: number,
  ): Promise<NouraSyncOpDownloadResponse> {
  ...
  private async _cfgOrError(): Promise<NouraSyncPrivateCfg> {
    const cfg = await this.privateCfg.load();
    if (!cfg) {
  ...
  private _resolveBaseUrl(cfg: NouraSyncPrivateCfg): string {
   ...
   * error response body. The NouraSync server's contract for the
  ...
  private _handleNativeRequestError(
    ...
    this._deps.logger.error(
      `${this._logLabel}: NouraSync request failed (native)`,
      undefined,
      {
        path,
        durationMs: duration,
    ...
    // `NouraSyncHttpStatusError` from the non-2xx branch) carry only
    ...
    if (
      ...
      error instanceof NouraSyncHttpStatusError
    ) {
      throw error;
    }
    if (error instanceof Error) {
      throw new Error(`NouraSync native request failed: ${error.name}`);
    }
    throw new Error(`NouraSync native request failed: ${toSyncLogError(error).name}`);
  }
  ...
  private async _doWebFetch<T>(
    ...
    for (let attempt = 0; attempt <= NOURASYNC_WEB_MAX_RETRIES; attempt++) {
      const startTime = Date.now();
      const controller = new AbortController();
      const timeoutId = setTimeout(
        () => controller.abort(),
        NOURASYNC_REQUEST_TIMEOUT_MS,
      );
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new NouraSyncHttpStatusError(
        ...
        const duration = Date.now() - startTime;
        if (duration > 30000) {
          this._deps.logger.warn(`${this._logLabel}: Slow NouraSync request detected`, {
            path,
            durationMs: duration,
        ...
        if (error instanceof Error && error.name === 'AbortError') {
          this._deps.logger.error(
            `${this._logLabel}: NouraSync request timeout`,
            undefined,
            {
              path,
              durationMs: duration,
              timeoutMs: NOURASYNC_REQUEST_TIMEOUT_MS,
            },
          );
          // Path is NOT interpolated — relative path includes
          // `excludeClient` (clientId) on download queries.
          throw new Error(
            `NouraSync request timeout after ${NOURASYNC_REQUEST_TIMEOUT_MS / 1000}s`,
          );
        }

        const isNetworkError = this._isRetryableWebRequestError(error);
        if (isNetworkError && attempt < NOURASYNC_WEB_MAX_RETRIES) {
          const delayMs = 1000 * (attempt + 1);
          this._deps.logger.warn(
            `${this._logLabel}: transient NouraSync request failure, retrying in ${delayMs}ms`,
            {
              path,
              attempt: attempt + 1,
              maxRetries: NOURASYNC_WEB_MAX_RETRIES,
              delayMs,
              durationMs: duration,
              ...this._getSafeErrorLogMeta(error),
            },
        ...
        this._deps.logger.error(
          `${this._logLabel}: NouraSync request failed`,
          undefined,
          {
            path,
            durationMs: duration,
    ...
    throw new Error('NouraSync request failed');
  }
  ...
  private async _doNativeFetch<T>(
    ...
    try {
      const response = await executeNativeRequestWithRetry(
        {
          ...
          readTimeout: NOURASYNC_REQUEST_TIMEOUT_MS,
        },
        {
      ...
      if (duration > 30000) {
        this._deps.logger.warn(
          `${this._logLabel}: Slow NouraSync request detected (native)`,
          {
            path,
            durationMs: duration,
            durationSec: (duration / 1000).toFixed(1),
