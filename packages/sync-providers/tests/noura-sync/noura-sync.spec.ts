...
import {
  ...
  NOURA_SYNC_DEFAULT_BASE_URL,
  NouraSyncProvider,
  type NouraSyncDeps,
  type NouraSyncPrivateCfg,
  type NouraSyncResponseValidators,
  type NouraSyncStorage,
} from '../../src/noura-sync';
...
const createValidatorsPassthrough = (): NouraSyncResponseValidators => ({
  validateOpUpload: (data) => data as OpUploadResponse,
  validateOpDownload: (data) => data as NouraSyncOpDownloadResponse,
...
const buildProvider = (
  ...
  const deps: NouraSyncDeps = {
    ...
    defaultBaseUrl: NOURA_SYNC_DEFAULT_BASE_URL,
    webRequestRetryDelay: webRequestRetryDelay as (ms: number) => Promise<void>,
  };
  const provider = new NouraSyncProvider(deps);
  return {
    provider,
    deps,
    cfgStore,
...
const testConfig: NouraSyncPrivateCfg = {
...
describe('NouraSyncProvider', () => {
  describe('properties', () => {
    it('has correct id', () => {
      const { provider } = buildProvider();
      expect(provider.id).toBe(PROVIDER_ID_NOURA_SYNC);
    });
  ...
  describe('getWebSocketParams', () => {
    ...
    it('falls back to the default base URL when none is configured', async () => {
      ...
      expect(await provider.getWebSocketParams()).toEqual({
        baseUrl: NOURA_SYNC_DEFAULT_BASE_URL,
        accessToken: 'test-access-token',
      });
    });
  });
  ...
  describe('error handling', () => {
    ...
    it('does NOT retry AbortError (fetch timeout) — surfaces timeout error', async () => {
      ...
      expect(caught?.message).toMatch(/^NouraSync request timeout/);
    });
  ...
  describe('authentication error handling', () => {
    ...
    it('AuthFailSPError does NOT retain response body on `additionalLog` (privacy invariant)', async () => {
      ...
      // `additionalLog` carries the constructor args. NouraSync constructs
  ...
  describe('native platform routing', () => {
    /**
     * The previous Jasmine spec used a `TestableNouraSyncProvider` subclass
    ...
    it('scrubs non-retryable foreign native errors (e.g. TLS cert) to name-only surface', async () => {
      ...
      expect(caught?.message).toBe('NouraSync native request failed: TlsError');
    ...
    it('scrubs foreign errors that coincidentally start with "HTTP " in the message', async () => {
      ...
      expect(caught?.message).toBe('NouraSync native request failed: NetworkPolicyError');
      expect(caught!.message).not.toContain('sync.example.com');
  ...
  describe('logger behavior', () => {
    ...
    it('logs error on fetch failure with safe meta only (no body, no URL)', async () => {
      ...
      expect(spy.error).toHaveBeenCalledWith(
        expect.stringContaining('NouraSync request failed'),
        undefined,
    ...
    it('logs timeout errors with path metadata (path is safe in structured log)', async () => {
      ...
      expect(spy.error).toHaveBeenCalledWith(
        expect.stringContaining('NouraSync request timeout'),
        undefined,
        expect.objectContaining({
          path: '/api/sync/ops?sinceSeq=0&excludeClient=client-1',
          timeoutMs: 75000,
  ...
  describe('privacy regression: no user content in error/log surface', () => {
    ...
    it('logger.error catch-paths receive structured meta (no raw error objects)', async () => {
      ...
      // SyncLogger.error signature is (msg, error?, meta?). NouraSync's
