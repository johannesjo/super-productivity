...
export const toDateStr = (date: Date): ISODate => {
...
const wholeWeeksBetween = (from: Date, to: Date): number =>
  Math.round((to.getTime() - from.getTime()) / (7 * 86_400_000));
...
const monthWeekday = (year: number, month: number, cfg: TaskRepeatCfg): Date => {
  const weekday = cfg.daysOfWeek[0] ?? 0;
  const weekNumber = cfg.weekOfMonth ?? 1;
  if (weekNumber >= 4) return lastWeekdayOfMonth(year, month, weekday);
  return nthWeekdayOfMonth(year, month, weekday, weekNumber);
};

const occurrenceInPeriod = (cfg: TaskRepeatCfg, year: number, month: number): Date => {
  if (cfg.repeatEveryUnit === 'MONTHLY') {
    if (cfg.daysOfWeek.length > 0) return monthWeekday(year, month, cfg);
    const day = Math.min(cfg.dayOfMonth ?? 1, daysInMonth(year, month));
    return new Date(Date.UTC(year, month, day));
  }
  // YEARLY
  const targetMonth = (cfg.yearMonth ?? 1) - 1;
  if (cfg.daysOfWeek.length > 0) return monthWeekday(year, targetMonth, cfg);
 ...
 */
export const getRepeatConfigNextDate = (
  cfg: TaskRepeatCfg,
  ...
  if (cfg.repeatEveryUnit === 'DAILY') {
    ...
    for (let week = 0; week < cfg.repeatEvery + 1; week += 1) {
      ...
      if (wholeWeeksBetween(anchor, weekStart) % cfg.repeatEvery !== 0) continue;
  ...
  if (cfg.repeatOffset) next = addDays(next, cfg.repeatOffset);
  const nextStr = toDateStr(next);
  if (cfg.endDate && nextStr > cfg.endDate) return undefined;
  return nextStr;
};
 ...
 */
export const expandRepeatConfig = (
  cfg: TaskRepeatCfg,
  rangeStart: ISODate,
  rangeEnd: ISODate,
): { dates: ISODate[] } => {
  ...
  while (guard < 10_000) {
    ...
    if (next >= rangeStart) dates.push(next);
    cursor = next;
    guard += 1;
  }
  return { dates };
