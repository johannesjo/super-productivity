...
const repeatCarryOver = (task: Task): Task => ({
  ...task,
  id: '',
  status: 'open',
  doneOn: undefined,
...
export const reduceDomain = (input: DomainState, command: DomainCommand): DomainState => {
  let state = input;
  switch (command.type) {
    case 'task/add':
    ...
    case 'note-bookmark/remove': {
      ...
      return {
        ...
        notes: {
          ...
          [note.id]: {
            ...note,
            bookmarks: note.bookmarks.filter(
              (bookmark) => bookmark.id !== command.payload.bookmarkId,
            ),
          },
        },
      };
    ...
    case 'counter/tick': {
      ...
      if (!counter || counter.counterType !== 'COUNTER') return state;
      return {
        ...state,
        counters: {
          ...state.counters,
          [counter.id]: {
            ...
            modifiedAt:
              command.payload.value > 0 ? counter.modifiedAt : counter.modifiedAt,
          },
        },
      };
    }
...
const addTask = (state: DomainState, task: Task): DomainState => {
...
const updateTask = (
  state: DomainState,
  payload: { id: string; patch: Partial<Omit<Task, 'id'>> },
): DomainState => patchTask(state, payload.id, payload.patch);

const toggleTask = (
  state: DomainState,
  payload: { id: string; doneOn: number },
): DomainState => {
...
const removeTask = (state: DomainState, id: string): DomainState => {
  ...
  for (const descendantId of descendants) delete tasks[descendantId];
  ...
  for (const entry of Object.values(state.trackedEntries)) {
    if (entry.taskId && descendants.has(entry.taskId)) delete trackedEntries[entry.id];
  }
  return {
    ...state,
    tasks,
    ...
    trackedEntries,
    selectedTaskId:
      state.selectedTaskId && descendants.has(state.selectedTaskId)
        ? undefined
        : state.selectedTaskId,
    activeSessionId:
      state.activeSessionId && trackedEntries[state.activeSessionId]
...
const setTaskStatus = (
  ...
  if (removeFromArchive) delete archives[id];
...
const repeatRollover = (
  state: DomainState,
  payload: { id: string; today: string },
): DomainState => {
  const current = state.tasks[payload.id];
  if (!current) return state;
...
const upsertEntity = <K extends keyof DomainState, E extends { id: string }>(
  state: DomainState,
  key: K,
  entity: E,
): DomainState => {
  const map = state[key] as unknown as Record<
    string,
    { id: string } & Record<string, unknown>
  >;
  const value = entity as unknown as { id: string } & Record<string, unknown>;
  return { ...state, [key]: { ...map, [value.id]: value } } as DomainState;
};

const updateEntity = <K extends keyof DomainState>(
  state: DomainState,
  key: K,
...
const removeEntity = <K extends keyof DomainState>(
  state: DomainState,
  key: K,
  id: string,
): DomainState => {
...
const addProject = (
  state: DomainState,
  project: DomainState['projects'][string],
): DomainState => {
  return {
    ...state,
...
const updateProject = (
  state: DomainState,
  payload: { id: string; patch: Partial<Omit<DomainState['projects'][string], 'id'>> },
): DomainState => {
...
const removeTag = (state: DomainState, id: string): DomainState => {
...
const removeRepeatCfg = (state: DomainState, id: string): DomainState => {
  if (!state.taskRepeatCfgs[id]) return state;
  const taskRepeatCfgs = { ...state.taskRepeatCfgs };
  delete taskRepeatCfgs[id];
  const tasks = { ...state.tasks };
  for (const task of Object.values(tasks)) {
    if (task.repeatCfgId === id) tasks[task.id] = { ...task, repeatCfgId: undefined };
  }
  return { ...state, taskRepeatCfgs, tasks };
};

const removeWorkContext = (state: DomainState, id: string): DomainState => {
  if (!state.workContexts[id]) return state;
  ...
  return {
    ...
    workContexts,
    activeWorkContextId:
      state.activeWorkContextId === id ? undefined : state.activeWorkContextId,
  };
};

// --- tracked entries / worklog ---

const startEntry = (state: DomainState, entry: TrackedEntry): DomainState => ({
  ...state,
...
const stopEntry = (
  state: DomainState,
  payload: { id: string; endedAt: number; durationMs: number },
): DomainState => {
  const entry = state.trackedEntries[payload.id];
  if (!entry) return state;
  const finished: TrackedEntry = {
    ...entry,
...
const addManualEntry = (state: DomainState, entry: TrackedEntry): DomainState =>
  attributeTime(state, entry, entry);
...
const attributeTime = (
  ...
  return {
    ...
    activeSessionId:
      state.activeSessionId === before.id ? undefined : state.activeSessionId,
  };
};

const addWorklogFromEntry = (state: DomainState, entry: TrackedEntry): DomainState => {
    ...
    counters: {
      ...
      [counter.id]: {
        ...
        startedOn: new Date(payload.at)
          .toISOString()
          .slice(0, 10) as import('./entities').ISODate,
        modifiedAt: payload.at,
      },
    },
  };
