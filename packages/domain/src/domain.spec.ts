...
const counterCfg = (overrides: Partial<TaskRepeatCfg> = {}): TaskRepeatCfg => ({
  id: 'cfg-daily',
  title: 'Every day',
  repeatEvery: 1,
  ...
  it('evaluates smart lists from their persisted criteria', () => {
    ...
    const list = {
      ...
      listConfig: {
        ...
        filterCriteria: [{ type: 'PRIORITY' as const, value: '2' }],
      },
      createdAt: 1,
      modifiedAt: 1,
    };
...
describe('recurrence engine', () => {
  ...
  it('honors repeatEvery for daily recurrence', () => {
    const next = getRepeatConfigNextDate(
      counterCfg({ repeatEvery: 2, repeatEveryUnit: 'DAILY' }),
      '2026-07-01',
    );
...
describe('migrateLegacyBackupToNoura', () => {
  ...
  it('clears reminders that were not active', () => {
    ...
    (withInactive.data.task.entities.parent as Record<string, unknown>).reminderActive =
      false;
...
describe('migrateDomainState v1 → v2', () => {
  ...
  it('moves sessions into tracked entries and fills new collections', () => {
    const state = importAnyState(v1) as DomainState;
