import { SyncDirection, SyncResult as BaseSyncResult, Task } from '../shared/types';
...
export function tasksToTree(tasks: Task[]): TreeNode[] {
  ...
  const visitedForCircular = new Set<string>();
  ...
  const hasCircularReference = (taskId: string, parentId: string): boolean => {
    visitedForCircular.clear();
    let currentId: string | null = parentId;

    while (currentId) {
      if (currentId === taskId || visitedForCircular.has(currentId)) {
        return true;
      }
      visitedForCircular.add(currentId);
  ...
  tasks.forEach((task) => {
    ...
    if (task.parentId && taskMap.has(task.parentId)) {
      ...
      if (!hasCircularReference(task.id, task.parentId)) {
    ...
    if (task.subTaskIds && task.subTaskIds.length > 0) {
      ...
      task.subTaskIds.forEach((subTaskId) => {
        ...
        if (child && !childrenAdded.has(subTaskId)) {
          ...
          if (!hasCircularReference(subTaskId, task.id)) {
    ...
    if (node.notes && node.children.length === 0) {
      ...
      noteLines.forEach((noteLine) => {
        ...
        if (checklistMatch) {
          ...
          // Check if line already starts with a dash to avoid double dashes
