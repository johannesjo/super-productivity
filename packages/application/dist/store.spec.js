...
describe('DomainStore', () => {
    it('persists one operation and only pushes local intent', async () => {
        ...
        await store.execute({ type: 'task/select', payload: { id: undefined } }, { operationId: 'local-op' });
        await store.execute({ type: 'task/select', payload: { id: undefined } }, { source: 'remote', operationId: 'remote-op' });
