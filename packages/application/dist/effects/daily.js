import { selectDoneOn, selectOpenTasks, selectTotalTrackedOn, } from '@noura/domain';
import { addDays, parseDate, toDateStr } from '@noura/domain';
/** `today` advanced by one ISO day (UTC-safe). */
export const nextDay = (today) => toDateStr(addDays(parseDate(today), 1));
/**
 ...
 */
export const buildFinishDayCommand = (state, today) => {
 ...
 */
export const buildPlanCommands = (state, today) => {
    const tomorrow = nextDay(today);
    return selectOpenTasks(state)
        .filter((task) => task.dueDay !== undefined && task.dueDay < today && !task.repeatCfgId)
        .map((task) => ({
        type: 'task/update',
 ...
 */
export const morningReview = (state, today) => {
    const overdue = selectOpenTasks(state).filter((task) => task.dueDay !== undefined && task.dueDay < today);
