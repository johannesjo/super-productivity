...
import { DomainSearchIndex, GlobalShortcuts, IdleDetection, NotificationService, ReminderScheduler, TakeABreak, TrackingReminder, } from './index';
...
const withTask = (state) => reduceDomain(state, { type: 'task/add', payload: { task: task() } });
...
describe('searchDomain', () => {
    ...
    it('finds tags and projects and respects the limit', () => {
        ...
        expect(results.some((result) => result.kind === 'tag' && result.title === 'reading')).toBe(true);
