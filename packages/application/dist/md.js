...
const inlinePatterns = [
    {
        re: /\*\*([^*]+)\*\*/g,
        replace: (_m, content) => `<strong>${content}</strong>`,
    },
    { re: /__([^_]+)__/g, replace: (_m, content) => `<strong>${content}</strong>` },
    {
        re: /(^|[^*])\*([^*\n]+)\*/g,
        replace: (_m, pre, content) => `${pre}<em>${content}</em>`,
    },
    {
        re: /(^|[^_])_([^_\n]+)_/g,
        replace: (_m, pre, content) => `${pre}<em>${content}</em>`,
    },
    { re: /~~([^~]+)~~/g, replace: (_m, content) => `<del>${content}</del>` },
    { re: /`([^`\n]+)`/g, replace: (_m, content) => `<code>${content}</code>` },
    {
        re: /\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g,
        replace: (_m, label, url) => `<a href="${escapeAttribute(url)}" rel="noopener noreferrer">${label}</a>`,
    },
];
const inline = (value) => {
    let result = escapeHtml(value);
    for (const { re, replace } of inlinePatterns)
...
export const markdownPreview = (source, maxLines = 2) => source
    .split(/\r?\n/)
