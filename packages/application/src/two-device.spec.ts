...
import {
  ...
  type NouraSyncOperationEndpoint,
} from './sync';
...
class InMemoryServer implements NouraSyncOperationEndpoint {
  nextSeq = 0;
  readonly operations: EncryptedServerOperation[] = [];
  ...
  async upload(operation: EncryptedServerOperation): Promise<{ serverSeq: number }> {
    if (this.operations.some((item) => item.id === operation.id)) {
      return {
        serverSeq:
          this.operations.find((item) => item.id === operation.id)?.serverSeq ?? 0,
      };
    }
    this.nextSeq += 1;
...
const pushTask = (id: string): DomainOperation => ({
  id: `${id}-op`,
  clientId: 'client-a',
  sequence: 1,
  timestamp: 1,
...
describe('two-device sync round trip (client transport, real encryption)', () => {
  ...
  it('never re-pushes remote operations back to the server', async () => {
    ...
    await b.store.execute(
      { type: 'task/update', payload: { id: 'task-3', patch: { title: 'Edited on B' } } },
      { operationId: 'b-op' },
    );
    await b.transport.sync();
