 ...
 */
export const buildCompletionHistoryCommand = (
  state: DomainState,
  task: Task,
  at: number,
): DomainCommand[] => {
  const day = new Date(at).toISOString().slice(0, 10) as ISODate;
  const doneToday = selectDoneOn(state, day).filter(
    (candidate) => candidate.id !== task.id,
  );
  if (task.status === 'done') doneToday.push(task);
  const totalTimeEstimate = doneToday.reduce(
    (total, current) => total + current.estimateMs,
    0,
  );
  return [
    {
 ...
 */
export const buildWorklogCommand = (entry: TrackedEntry): DomainCommand[] => {
 ...
 */
export const focusDaySummary = (state: DomainState, day: ISODate) => {
  const entries = Object.values(state.trackedEntries).filter(
    (entry) =>
      entry.date === day || new Date(entry.startedAt).toISOString().slice(0, 10) === day,
  );
