...
import { createInitialState, reduceDomain, type Task } from '@noura/domain';
import {
  buildCompletionHistoryCommand,
  buildFinishDayCommand,
  buildPlanCommands,
  buildWorklogCommand,
  focusDaySummary,
  morningReview,
...
describe('focus effects', () => {
  it('records a completed task into the day history', () => {
    ...
    expect(
      command?.type === 'history/record' && command.payload.entry.totalTimeEstimate,
    ).toBe(30_000);
  });
