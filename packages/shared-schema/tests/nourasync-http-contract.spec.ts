...
} from '../src/nourasync-http-contract';
...
describe('NouraSync HTTP contract schemas', () => {
  it('validates ops upload requests with the shared server limit', () => {
    const parsed = NouraSyncUploadOpsRequestSchema.parse({
    ...
    expect(NOURA_SYNC_MAX_OPS_PER_UPLOAD).toBe(100);
  });
  ...
  it('passes oversized entityIds through for per-operation validation', () => {
    const operation = {
      ...
      entityIds: Array.from(
        { length: NOURA_SYNC_MAX_ENTITY_IDS_PER_OP + 1 },
        (_, i) => `task-${i}`,
      ),
    };
    ...
    expect(parsed.ops[0].entityIds).toHaveLength(NOURA_SYNC_MAX_ENTITY_IDS_PER_OP + 1);
    expect(() => NouraSyncOperationSchema.parse(operation)).toThrow();
  });

  it('rejects invalid client IDs in upload requests', () => {
    expect(() =>
      NouraSyncUploadOpsRequestSchema.parse({
  ...
  it.each([0, 1.5, -1, 101])(
    ...
    (schemaVersion) => {
      ...
      expect(() =>
        NouraSyncOperationSchema.parse({
          ...createValidOperation(),
          schemaVersion,
        }),
      ).toThrow();
  ...
  it.each([0, 1.5, -1, 101])(
    ...
    (schemaVersion) => {
      expect(() =>
        NouraSyncUploadSnapshotRequestSchema.parse({
          state: {},
          clientId: 'client_1',
          reason: 'recovery',
  ...
  it('coerces download query numbers like the route-level schema', () => {
    const parsed = NouraSyncDownloadOpsQuerySchema.parse({
      sinceSeq: '5',
      limit: '50',
  ...
  it('validates snapshot upload requests', () => {
    const parsed = NouraSyncUploadSnapshotRequestSchema.parse({
      state: { tasks: {} },
      clientId: 'client_1',
      reason: 'recovery',
  ...
  it('accepts legacy repair snapshots and preserves a causal base when present', () => {
    ...
    expect(NouraSyncUploadSnapshotRequestSchema.parse(repairRequest)).toEqual(
      repairRequest,
    );
  ...
  it('preserves causal repair metadata on downloaded operations', () => {
    const parsed = NouraSyncOperationSchema.parse({
      ...createValidOperation(),
      opType: 'REPAIR',
      repairBaseServerSeq: 42,
    });
  ...
  it('preserves causal repair capability negotiation on downloads', () => {
    const parsed = NouraSyncDownloadOpsResponseSchema.parse({
      ops: [],
      hasMore: false,
      latestSeq: 0,
