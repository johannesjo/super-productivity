...
- **Full dump** (`nourasync_*.sql.gz`) — complete database including all operations (~300MB+ for active instances)
- **Accounts-only dump** (`nourasync_accounts_*.sql.gz`) — just `users` and `passkeys` tables (tiny, <1MB)
...
(crontab -l 2>/dev/null; echo "0 3 * * * RETENTION_DAYS=3 /path/to/scripts/backup.sh >> /var/log/nourasync-backup.log 2>&1") | crontab -
```
...
| `DB_CONTAINER`   | `nourasync-postgres` | Docker container name                      |
| `POSTGRES_USER`  | `nourasync`          | Database user                              |
| `POSTGRES_DB`    | `nourasync`          | Database name                              |
...
RCLONE_REMOTE=b2:my-bucket/nourasync ./scripts/backup.sh --upload
```
...
gunzip -c backups/nourasync_accounts_YYYYMMDD_HHMMSS.sql.gz | \
  docker exec -i nourasync-postgres psql -U nourasync nourasync
...
- Verified by e2e tests (`nourasync-server-backup-revert.spec.ts`)
...
```bash
# 1. Stop the server
docker compose stop nourasync

# 2. Drop existing data and restore the full dump
docker exec -i nourasync-postgres psql -U nourasync nourasync \
  -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
gunzip -c backups/nourasync_YYYYMMDD_HHMMSS.sql.gz | \
  docker exec -i nourasync-postgres psql -U nourasync nourasync

# 3. Restart the server
docker compose start nourasync
```

> **Note:** The database name (`nourasync` above) must match your deployment's
...
```bash
DATABASE_URL=... bun run recover-user -- --user <email|id> --inspect
```
...
- Run it from a dev checkout with Bun; the production image intentionally does
  not include administration tooling. Pointing `DATABASE_URL` at a restored
...
gunzip -c backups/nourasync_YYYYMMDD_HHMMSS.sql.gz | head -5

# Check cron is running
cat /var/log/nourasync-backup.log
```

## E2E Test Coverage

The backup recovery scenarios are covered by automated tests in `e2e/tests/sync/nourasync-server-backup-revert.spec.ts`:
