#!/usr/bin/env tsx
/**
 * NouraSync Storage Analysis Tool
 *
 * A reusable investigation script for analyzing operation patterns, user behavior,
 * and storage anomalies in the NouraSync database.
 *
 * Usage:
 *   bun run analyze-storage -- <command> [options]
 *
 * Commands:
...
const analyzeOperationSizes = async (userId?: number): Promise<void> => {
  ...
  const sizeDistribution: any[] = await db.$queryRaw`
    SELECT
...
const analyzeOperationTimeline = async (userId?: number): Promise<void> => {
  ...
  const perDay: any[] = await db.$queryRaw`
    SELECT
  ...
  console.log('\nOperations per Hour (last 24 hours):');
  const perHour: any[] = await db.$queryRaw`
    SELECT
...
const analyzeOperationTypes = async (userId?: number): Promise<void> => {
  ...
  const byOpType: any[] = await db.$queryRaw`
    SELECT
      op_type,
      COUNT(*) as count,
      SUM(pg_column_size(payload)) as total_bytes,
  ...
  // By entityType
  console.log('\nBy Entity Type:');
  const byEntityType: any[] = await db.$queryRaw`
    SELECT
      entity_type,
      COUNT(*) as count,
      SUM(pg_column_size(payload)) as total_bytes,
  ...
  const byActionType: any[] = await db.$queryRaw`
    SELECT
      action_type,
      COUNT(*) as count,
      SUM(pg_column_size(payload)) as total_bytes,
...
const analyzeLargeOperations = async (limit = 20): Promise<void> => {
  ...
  const largeOps: any[] = await db.$queryRaw`
    SELECT
      o.id,
      o.user_id,
      u.email,
  ...
  if (largeOps.length > 0) {
    ...
    const fullOp = await db.operation.findUnique({
      where: { id: largest.id },
      select: { payload: true },
    });
...
const detectRapidFire = async (thresholdPerSecond = 5): Promise<void> => {
  ...
  const rapidFire: any[] = await db.$queryRaw`
    SELECT
      user_id,
      client_id,
...
const analyzeSnapshots = async (): Promise<void> => {
  ...
  const snapshotStats: any[] = await db.$queryRaw`
    SELECT
  ...
  const correlation: any[] = await db.$queryRaw`
    SELECT
      u.id,
      u.email,
...
const userDeepDive = async (userId: number): Promise<void> => {
  ...
  const [syncState, devices] = await Promise.all([
    db.userSyncState.findUnique({ where: { userId } }),
    db.syncDevice.findMany({ where: { userId } }),
  ]);
  ...
  if (syncState) {
    console.log('\n--- Sync State ---');
    console.log(`Last Seq: ${syncState.lastSeq}`);
    console.log(`Last Snapshot Seq: ${syncState.lastSnapshotSeq || 'N/A'}`);
    console.log(
      `Snapshot Size: ${formatBytes(syncState.snapshotData?.length || 0)}`,
    );
    console.log(
      `Snapshot Age: ${syncState.snapshotAt ? formatDuration(Date.now() - Number(syncState.snapshotAt)) : 'N/A'}`,
    );
  }
  ...
  const perDevice: any[] = await db.$queryRaw`
    SELECT
      client_id,
      COUNT(*) as ops,
      SUM(pg_column_size(payload)) as total_bytes,
...
const compareUsers = async (userId1: number, userId2: number): Promise<void> => {
  ...
  const [user1, user2] = await Promise.all([
    db.user.findUnique({ where: { id: userId1 }, select: { email: true } }),
    db.user.findUnique({ where: { id: userId2 }, select: { email: true } }),
  ]);
...
Examples:
  bun run analyze-storage -- operation-sizes --user 29
  bun run analyze-storage -- large-ops --limit 50
  bun run analyze-storage -- rapid-fire --threshold 10
  bun run analyze-storage -- user-deep-dive --user 27
  bun run analyze-storage -- compare-users 27 29
  `);
};
