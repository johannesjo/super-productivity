...
async function main() {
  ...
  if (!target) {
    console.log('Usage: bun run clear-data -- <email|--all>');
  ...
  if (target === '--all') {
    ...
    try {
      ...
      await db.$transaction([
        db.operation.deleteMany({ where: { userId: user.id } }),
        db.userSyncState.deleteMany({ where: { userId: user.id } }),
        db.syncDevice.deleteMany({ where: { userId: user.id } }),
      ]);
