...
describe('SnapshotService', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(db.$queryRaw).mockResolvedValue([{ bytes: 0 }] as any);
  ...
  describe('getCachedSnapshot', () => {
    it('should return null when no snapshot exists', async () => {
      vi.mocked(db.userSyncState.findUnique).mockResolvedValue(null);
    ...
    it('should return null when snapshotData is null', async () => {
      vi.mocked(db.userSyncState.findUnique).mockResolvedValue({
        snapshotData: null,
        lastSnapshotSeq: 10,
    ...
    it('should return null and invalidate the cached blob on decompression error', async () => {
      ...
      vi.mocked(db.userSyncState.update).mockResolvedValue({} as any);

      const result = await service.getCachedSnapshot(1);

      expect(result).toBeNull();
      expect(db.userSyncState.update).toHaveBeenCalledWith({
        where: { userId: 1 },
        data: {
          snapshotData: null,
  ...
  describe('getCachedSnapshotGeneratedAt', () => {
    ...
    it('should read only snapshot metadata', async () => {
      ...
      expect(db.userSyncState.findUnique).toHaveBeenCalledWith({
        where: { userId: 1 },
  ...
  describe('cacheSnapshot', () => {
    it('should compress and conditionally update an existing row', async () => {
      ...
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 1 } as any);

      const result = await service.cacheSnapshot(1, state, 10);

      expect(db.userSyncState.updateMany).toHaveBeenCalledWith({
        where: {
          userId: 1,
      ...
      expect(db.userSyncState.create).not.toHaveBeenCalled();
    });
    ...
    it('should report replacement byte delta when caching snapshot', async () => {
      ...
      vi.mocked(db.$queryRaw).mockResolvedValue([{ bytes: BigInt(5) }] as any);
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 1 } as any);
    ...
    it('should create the row when no userSyncState exists (first-time user)', async () => {
      const state = { TASK: { 'task-1': { id: 'task-1' } } };
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 0 } as any);
      vi.mocked(db.userSyncState.create).mockResolvedValue({} as any);

      const result = await service.cacheSnapshot(1, state, 10);

      expect(db.userSyncState.create).toHaveBeenCalledWith({
    ...
    it('should swallow 23505 when a concurrent writer inserted the row first', async () => {
      const state = { TASK: { 'task-1': { id: 'task-1' } } };
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 0 } as any);
      const uniqueViolation = Object.assign(new Error('unique constraint'), {
        code: '23505',
      });
      vi.mocked(db.userSyncState.create).mockRejectedValue(uniqueViolation);
    ...
    it('should rethrow non-23505 create errors', async () => {
      const state = { TASK: { 'task-1': { id: 'task-1' } } };
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 0 } as any);
      vi.mocked(db.userSyncState.create).mockRejectedValue(new Error('boom'));
    ...
    it('should be a no-op when a newer snapshot already won the race', async () => {
      ...
      // throws 23505 because a row exists. Both swallowed.
      const state = { TASK: { 'task-1': { id: 'task-1' } } };
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 0 } as any);
      const uniqueViolation = Object.assign(new Error('unique constraint'), {
        code: '23505',
    ...
    it('should clear a previously-cached snapshot when the new one is oversized', async () => {
      ...
      vi.mocked(db.$queryRaw).mockResolvedValue([{ bytes: BigInt(8000) }] as any);
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 1 } as any);
    ...
    it('should not touch the row when oversized and no previous snapshot exists', async () => {
      ...
      vi.mocked(db.$queryRaw).mockResolvedValue([{ bytes: BigInt(0) }] as any);
      vi.mocked(db.userSyncState.updateMany).mockResolvedValue({ count: 0 } as any);
      ...
      expect(db.userSyncState.updateMany).not.toHaveBeenCalled();
      expect(result).toEqual({
        cached: false,
        bytesWritten: 0,
        previousBytes: 0,
  ...
  describe('cacheSnapshotIfReplayable', () => {
    ...
    it('should cache plaintext payloads', async () => {
      ...
      expect(db.userSyncState.updateMany).toHaveBeenCalled();
    });
  });
  ...
  describe('getRestorePoints', () => {
    it('should return empty array when no restore points exist', async () => {
      vi.mocked(db.operation.findMany).mockResolvedValue([]);
