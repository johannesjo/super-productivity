...
describeWithDb('causal REPAIR serialization (PostgreSQL)', () => {
  ...
  afterAll(async () => {
    await db.operation.deleteMany({ where: { userId } });
    await db.syncDevice.deleteMany({ where: { userId } });
    await db.userSyncState.deleteMany({ where: { userId } });
