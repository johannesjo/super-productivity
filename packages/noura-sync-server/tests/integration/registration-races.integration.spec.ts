...
describeWithDb('Registration races (PostgreSQL)', () => {
  ...
  it('activates only the passkey bound to the verification link', async () => {
    ...
    const storedUser = await observer.user.findUniqueOrThrow({ where: { email } });
    const passkeys = await observer.passkey.findMany({
      where: { userId: storedUser.id },
    });
    expect(passkeys).toHaveLength(1);
    expect(passkeys[0].credentialId).toEqual(ownerCredentialId);
  ...
  it('keeps a passkey registration valid when a concurrent email delivery fails', async () => {
    ...
    expect(passkeys).toHaveLength(1);
    expect(passkeys[0].credentialId).toEqual(secondCredentialId);
    expect(storedUser?.isVerified).toBe(1);
  });
  ...
  it('does not let a later passkey attempt invalidate a magic-link verification', async () => {
    ...
    const storedUser = await observer.user.findUniqueOrThrow({ where: { email } });
    const [passkeys, pendingRegistrations] = await Promise.all([
      observer.passkey.findMany({ where: { userId: storedUser.id } }),
      observer.pendingPasskeyRegistration.findMany({
        where: { userId: storedUser.id },
      }),
    ]);
    expect(storedUser.isVerified).toBe(1);
    expect(passkeys).toHaveLength(0);
    expect(pendingRegistrations).toHaveLength(0);
  });
});
