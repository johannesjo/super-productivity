 ...
 *
 * This file provides a lightweight in-memory database facade for isolated
 * service tests. PostgreSQL behavior is covered by the integration suite.
 */
...
// Compatibility surface used by a few service-level test helpers.
const createMockDb = () => {
  const mockDb = {
    prepare: (sql: string) => {
      return {
...
// Export mock functions used by service-level tests.
...
vi.mock('../src/db', () => {
  ...
  // Create database mock with all needed operations
  const dbMock = {
    $transaction: vi.fn().mockImplementation(async (callback: any) => {
      ...
      const tx = {
        ...
        userSyncState: {
          ...
          update: vi.fn().mockImplementation(async (args: any) => {
            ...
            if (existing) {
              ...
              // Handle database facade increment syntax: { lastSeq: { increment: 1 } }
        ...
        syncDevice: {
          upsert: vi.fn().mockImplementation(async (args: any) => {
            // Handle both key naming conventions (database facade uses userId_clientId)
