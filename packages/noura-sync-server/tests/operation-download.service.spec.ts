...
// Helper to create a mock operation row (as returned by the database)
...
// operation.aggregate({ _min }) call whose database-generated `OFFSET 0`
...
describe('OperationDownloadService', () => {
  ...
  describe('getOpsSinceWithSeq', () => {
    ...
    it('should select only download response fields inside atomic reads', async () => {
      ...
      vi.mocked(db.$transaction).mockImplementation(async (fn: any, options: any) => {
    ...
    it('uses an indexed findFirst (not the slow aggregate) for minSeq', async () => {
    ...
    it('should apply limitVectorClockSize to aggregate result exceeding MAX', async () => {
      ...
      vi.mocked(db.$queryRaw).mockResolvedValue(clockRows);
