...
describe('Passkey Authentication', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.passkey.count.mockResolvedValue(1);
    mockDb.pendingPasskeyRegistration.findUnique.mockResolvedValue(null);
    mockDb.pendingPasskeyRegistration.create.mockResolvedValue({});
  ...
  describe('Registration Flow', () => {
    ...
    it('should verify registration and create new user', async () => {
      ...
      expect(mockDb.pendingPasskeyRegistration.create).toHaveBeenCalledWith({
    ...
    it('should keep a failed-delivery credential pending and inactive', async () => {
      ...
      expect(result).toEqual(registrationResponse);
      expect(mockDb.pendingPasskeyRegistration.create).toHaveBeenCalledOnce();
      expect(mockDb.passkey.create).not.toHaveBeenCalled();
  ...
  describe('Authentication Flow', () => {
    ...
    it('should verify authentication and return user info', async () => {
      ...
      mockDb.passkey.findUnique.mockResolvedValue({
        ...mockPasskey,
        user: {
          id: 1,
          email: testEmail,
      ...
      mockDb.passkey.update.mockResolvedValue({});
      ...
      expect(mockDb.passkey.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            counter: BigInt(1),
          }),
    ...
    it('should reject authentication with wrong passkey', async () => {
      ...
      mockDb.passkey.findUnique.mockResolvedValue(null);
  ...
  describe('Recovery Flow', () => {
    ...
    it('should return the neutral response and clear only its claimed token when email fails', async () => {
      ...
      expect(result).toEqual(recoveryResponse);
      const claimedToken =
        mockDb.user.updateMany.mock.calls[0][0].data.passkeyRecoveryToken;
