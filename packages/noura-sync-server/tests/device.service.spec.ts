...
describe('DeviceService', () => {
  ...
  describe('isDeviceOwner', () => {
    ...
    it('should return true when multiple devices match (edge case)', async () => {
      vi.mocked(db.syncDevice.count).mockResolvedValue(2);
  ...
  describe('getAllUserIds', () => {
    it('should return empty array when no users exist', async () => {
      vi.mocked(db.userSyncState.findMany).mockResolvedValue([]);

      const result = await service.getAllUserIds();

      expect(result).toEqual([]);
      expect(db.userSyncState.findMany).toHaveBeenCalledWith({
        select: { userId: true },
        distinct: ['userId'],
      });
    });
    ...
    it('should handle single user', async () => {
      vi.mocked(db.userSyncState.findMany).mockResolvedValue([{ userId: 42 }] as any);
  ...
  describe('getOnlineDeviceCount', () => {
    it('should return count of online devices', async () => {
      ...
      vi.mocked(db.syncDevice.count).mockResolvedValue(3);
