...
// Mock the database module with database mocks
vi.mock('../src/db', async () => {
  ...
  const { UniqueViolationError } = await import('./sql-state-error');
  const { extractDrizzleSqlParameters } = await import('./drizzle-sql');
  ...
  const createTxMock = () => ({
    operation: {
      create: vi.fn().mockImplementation(async (args: any) => {
        ...
        if (state.operations.has(args.data.id)) {
          throw new UniqueViolationError('Unique constraint failed', {
            code: '23505',
            clientVersion: '5.0.0',
          });
        }
    ...
    userSyncState: {
      ...
      update: vi.fn().mockImplementation(async (args: any) => {
        ...
        if (existing) {
          ...
          // Handle database facade increment syntax
    ...
    $queryRaw: vi.fn().mockImplementation(async (strings: any, ...params: any[]) => {
      ...
      if (sql.includes('JOIN (VALUES')) {
        ...
        // parameters hold the flattened (entity_type, entity_id) touched pairs. The
        ...
        const valuesParam = params.find(
          (p: unknown) =>
            !!p &&
            typeof p === 'object' &&
            Array.isArray((p as { queryChunks?: unknown[] }).queryChunks),
        );
        const touchedParams = extractDrizzleSqlParameters(valuesParam);
...
describe('SyncService', () => {
  ...
  describe('uploadOps', () => {
    ...
    it('should chunk large batch entity prefetch queries', async () => {
      ...
      await prefetchLatestEntityOpsForBatch(userId, entityPairs, tx as unknown as never);
    ...
    it('should classify batch createMany PostgreSQL unique-violation stale-prefetch races as retryable', async () => {
      const service = new SyncService({ batchUpload: true });
      const uniqueViolation = new UniqueViolationError(
      ...
      const tx = {
        operation: {
          ...
          createMany: vi.fn().mockRejectedValue(uniqueViolation),
        },
        userSyncState: {
          updateMany: vi.fn(),
        },
      ...
      vi.mocked(db.$transaction).mockImplementationOnce(async (callback) =>
        callback(tx as unknown as never),
      );
  ...
  describe('cleanup', () => {
    ...
    it('should preserve the latest full-state operation and its replay tail', async () => {
      ...
      expect(db.operation.findFirst).toHaveBeenCalled();
