...
describe('loadConfigFromEnv - NouraSync batch uploads', () => {
  beforeEach(() => {
    resetEnv();
  });
  ...
  it('should enable batch uploads by default for a fresh database', async () => {
    const { loadConfigFromEnv } = await importConfig();
    const config = loadConfigFromEnv();
  ...
  it('should allow batch uploads to be disabled explicitly', async () => {
    process.env.NOURASYNC_BATCH_UPLOAD = 'false';
...
describe('DEFAULT_CORS_ORIGINS', () => {
  ...
  it('should allow the local web and Tauri client origins', async () => {
    const { loadConfigFromEnv } = await importConfig();
    const config = loadConfigFromEnv();
