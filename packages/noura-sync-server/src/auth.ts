...
export const verifyEmail = async (token: string): Promise<boolean> => {
  const pendingPasskey = await db.pendingPasskeyRegistration.findUnique({
...
export type TokenVerificationResult =
  { valid: true; userId: number; email: string } | { valid: false; reason: string };
...
export const verifyLoginMagicLink = async (
  ...
  const consume = await db.user.updateMany({
    where: {
      id: user.id,
      loginToken: token,
