    ...
    const result = await db.$transaction(
      async (tx) => {
        ...
        if (cachedRow?.snapshotData) {
          try {
            const decompressed = (
              await gunzipAsync(Buffer.from(cachedRow.snapshotData), {
        ...
        // user), fall back to `create` and swallow SQLSTATE 23505 if another writer
        ...
        if (compressed.length <= effectiveCap) {
          ...
          if (updateResult.count > 0) {
            ...
            // SQLSTATE 23505 and we treat that as "newer snapshot won".
            try {
