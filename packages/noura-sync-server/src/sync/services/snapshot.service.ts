...
export class SnapshotService {
  ...
  async getCachedSnapshotBytes(userId: number): Promise<number> {
    const result = await db.$queryRaw<[{ bytes: number | bigint | null }]>`
  ...
  async getCachedSnapshotGeneratedAt(userId: number): Promise<number | null> {
    const row = await db.userSyncState.findUnique({
      where: { userId },
  ...
  async getCachedSnapshot(userId: number): Promise<SnapshotResult | null> {
    ...
    try {
      const decompressed = (
        await gunzipAsync(Buffer.from(row.snapshotData), {
  ...
  async cacheSnapshot(
    ...
    if (!prepared.cacheable) {
      ...
      let cleared = 0;
      if (previousBytes > 0) {
        const clearResult = await db.userSyncState.updateMany({
          where: {
            userId,
    ...
    const updateResult = await db.userSyncState.updateMany({
      where: {
        userId,
        OR: [{ lastSnapshotSeq: null }, { lastSnapshotSeq: { lt: serverSeq } }],
      },
    ...
    // the updateMany and now, the unique-userId constraint reports SQLSTATE 23505
    // and we treat that as "newer snapshot won; nothing to do".
    try {
      await db.userSyncState.create({ data: { userId, ...data } });
      return {
        cached: true,
        bytesWritten: prepared.bytes,
        previousBytes,
