...
export class OperationDownloadService {
  ...
  async getOpsSinceWithSeq(
    ...
    const result = await db.$transaction(
      async (tx) => {
        ...
        if (sinceSeq > 0 && latestSeq > 0) {
          ...
          // NOTE: Drizzle's `aggregate({ _min })` compiles to
    ...
    ); // Matches other sync transactions; stays below Hono's 80s request timeout.
  ...
  private async _computeSnapshotVectorClock(
    ...
    // is slow enough to trip Drizzle's transaction timeout. Bounded by
