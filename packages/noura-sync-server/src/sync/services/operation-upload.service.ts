import type { DatabaseTransaction, DbJsonValue } from '../../db';
...
export class OperationUploadService {
  ...
  private async persistMergedFullStateClock(
    ...
    await tx.userSyncState.update({
      ...
      data: {
        ...
        latestFullStateVectorClock: mergedClock as DbJsonValue,
      },
    });
  }
  ...
  private async reserveSeqAndInsert(
    ...
    await tx.operation.createMany({
      data: accepted.map((candidate) => ({
        ...
        payload: candidate.op.payload as DbJsonValue,
        payloadBytes: BigInt(candidate.storageBytes),
        vectorClock: candidate.op.vectorClock as DbJsonValue,
  ...
  async processOperation(
      ...
      data: [
        {
          ...
          payload: op.payload as DbJsonValue,
          payloadBytes: BigInt(sized.bytes),
          vectorClock: op.vectorClock as DbJsonValue,
    ...
    // instead of aborting the PostgreSQL transaction with 23505/25P02.
