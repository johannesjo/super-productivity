...
export class StorageQuotaService {
   ...
   *
   * NouraSync starts from a fresh schema where every operation records
   * payload_bytes at write time, so reconciliation is a direct SUM with no
   * legacy fallback or backfill state.
   */
  async calculateStorageUsage(userId: number): Promise<{
    ...
    const usageResult = await db.$queryRaw<
      [
        {
    ...
    >`
      ...
      FROM (
        SELECT
          COALESCE(SUM(payload_bytes), 0)::bigint AS operations_bytes
        FROM operations
        WHERE user_id = ${userId}
      ) AS ops
    `;
   ...
   * zero. Uses $executeRaw for the GREATEST(...) clamp — Drizzle's `decrement`
  ...
  async deleteOldSyncedOpsForAllUsers(
    ...
    const states = await db.userSyncState.findMany({
      where: {
    ...
    for (const state of states) {
      ...
      // The cached marker is advisory. Confirm that it still points at a causal
      // full-state operation before authorizing destructive history pruning.
      // This protects against corruption and partial writes.
      if (protectedFromSeq !== null) {
        const markerIsCausal = await db.operation.findFirst({
          where: {
            userId: state.userId,
      ...
      if (protectedFromSeq === null) {
        const latestCoveredFullStateOp = await db.operation.findFirst({
          where: {
            userId: state.userId,
  ...
  private async deleteOldSyncedOpsBatch(
    ...
    const doomedOps = await db.operation.findMany({
      where: {
        userId,
    ...
    const result = await db.operation.deleteMany({
      where: {
        userId,
  ...
  async deleteOldestRestorePointAndOps(
    ...
    const restorePoints = await db.operation.findMany({
      where: {
        userId,
        ...CAUSAL_FULL_STATE_OPERATION_WHERE,
      },
    ...
    const fullStateRows = await db.$queryRaw<
    ...
    if (result.count > 0) {
      ...
      const cachedRow = await db.userSyncState.findUnique({
        where: { userId },
