...
const isRetryableOperationUniqueViolation = (error: unknown): boolean => {
  ...
  async uploadOps(
    ...
    try {
      ...
      await db.$transaction(
        async (tx) => {
          ...
          await tx.syncDevice.upsert({
            where: {
              // Drizzle composite key naming uses underscores; allow it here
        ...
        },
        {
          ...
          // Default Drizzle timeout (5s) is too short for these. Use 60s to match generateSnapshot.
          timeout: 60000,
      ...
      const isSerializationFailure =
        ['40001', '40P01'].includes(errorCode(err) ?? '') ||
      ...
      // Drizzle exceptions can include SQL fragments, column names, and FK names,
