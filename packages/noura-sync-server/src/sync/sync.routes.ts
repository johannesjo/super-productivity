import { HttpApp, HttpReply, HttpRequest } from '../http';
import { NouraSyncDownloadOpsQuerySchema } from '@sp/shared-schema';
...
export const syncRoutes = async (app: HttpApp): Promise<void> => {
  ...
  app.post<{ Body: UploadOpsRequest }>(
    '/ops',
    {
  ...
  app.get<{
    ...
    async (
      ...
      try {
        ...
        const parseResult = NouraSyncDownloadOpsQuerySchema.safeParse(req.query);
  ...
  app.get(
    ...
    async (req: HttpRequest, reply: HttpReply) => {
      try {
        const userId = getAuthUser(req).userId;
        const syncService = getSyncService();
