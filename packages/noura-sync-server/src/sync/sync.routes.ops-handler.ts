...
import { NouraSyncUploadOpsRequestSchema } from '@sp/shared-schema';
...
export const uploadOpsHandler = async (
  req: HttpRequest<{ Body: UploadOpsRequest }>,
  reply: HttpReply,
  ...
  try {
    ...
    const parseResult = NouraSyncUploadOpsRequestSchema.safeParse(body);
