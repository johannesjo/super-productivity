...
export const sendVerificationEmail = async (
  ...
  try {
    ...
    const config = loadConfigFromEnv();
    const from = config.smtp?.from || '"NouraSync" <noreply@example.com>';
    ...
    const info = await mailTransporter.sendMail({
      from,
      to,
      subject: 'Verify your NouraSync account',
      text:
...
export const sendPasskeyRecoveryEmail = async (
  ...
  try {
    ...
    const info = await mailTransporter.sendMail({
      from,
      to,
      subject: 'Recover your NouraSync passkey',
      text:
...
export const sendLoginMagicLinkEmail = async (
  ...
  try {
    ...
    const info = await mailTransporter.sendMail({
      from,
      to,
      subject: 'Your NouraSync login link',
      text:
        `Click the following link to log in to NouraSync: ${loginLink}\n\n` +
        `If you did not request this, please ignore this email.\n\n` +
        `This link will expire in 15 minutes.`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Login to NouraSync</h2>
