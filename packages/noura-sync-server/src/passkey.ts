...
export const verifyRegistration = async (
  ...
  try {
    ...
    const pendingCreated = await db.$transaction(async (tx) => {
      let userId: number;
      if (existingUser) {
...
export const generateAuthenticationOptions = async (
  ...
  const passkeyCount = user ? await db.passkey.count({ where: { userId: user.id } }) : 0;
...
export const requestPasskeyRecovery = async (
  ...
  if ((await db.passkey.count({ where: { userId: user.id } })) === 0) {
