...
- **Disable all animations (default: off)** — Turns off every transition and
  animation in the app. This is forced on, regardless of the setting, whenever
  the operating system reports a "reduce motion" preference, so the app follows
  the system accessibility setting without any configuration.
