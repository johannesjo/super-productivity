...
2. Open **Settings → Sync & Export → Import/Export** and select **Export data**.
3. Repeat for every profile.
4. Switch back to the profile you want to continue using before updating.

Use the normal **Export data** action after switching so the export captures
that profile's current data. Store the exported JSON files securely because
they can contain private data and credentials. Importing one later replaces the
current application data; it does not merge profiles.
