...
- **Disable all animations (default: off)** — Turns off every transition and
  animation in the app. Animations are also off automatically while your
  operating system requests reduced motion, so no configuration is needed. In
  that case the checkbox still shows unchecked and ticking it has no visible
  effect; change the system setting to get animations back. Changing the system
  setting mid-session applies to transitions right away, but parts of the
  interface that were already on screen keep their original animation setting
