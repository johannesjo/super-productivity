...
- **Disable all animations (default: off)** — Turns off every transition and
  ...
  operating system requests reduced motion, so the app follows the system
  accessibility setting without any configuration. In that case the checkbox
  still shows unchecked and ticking it off has no visible effect; change the
  system setting to get animations back.
