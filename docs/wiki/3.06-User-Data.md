...
- **sp-local-drafts** (version 1): Device-local **crash-safe editor drafts**. One object store (`drafts`), one row per in-progress edit, keyed by `${profileId}:${entityType}:${entityId}`. Backs the fullscreen note editor so unsaved edits survive a crash or force-close. Deliberately separate from `SUP_OPS`, **never synced or uploaded** and **excluded from backups and exports** — a draft is transient device state, not application data. It is reclaimed once redundant: cleared when the note is deleted on this device, cleared for the active profile when its dataset is replaced wholesale (JSON import, SuperSync restore), and otherwise pruned once older than 14 days or beyond the newest 200 entries per profile — on app start, when a note is opened, and on the save path once a profile grows past that cap, so retention holds in long-running sessions too. A draft that is saved or discarded is retained (no longer offered for recovery) until it ages out on that same 14-day clock. A profile's drafts are also deleted when that profile is deleted; switching profiles keeps each profile's drafts intact.
