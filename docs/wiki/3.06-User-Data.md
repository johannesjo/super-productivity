...
- If migrating the legacy `pf` database fails, the migration dialog reports the error and waits. Once the automatic pre-migration backup has been downloaded, the dialog also offers **Delete old data**, which — after a second confirmation — discards the legacy database and restarts the app empty so that backup can be imported. The option is hidden when the backup could not be created, since the legacy database is then the only remaining copy.
