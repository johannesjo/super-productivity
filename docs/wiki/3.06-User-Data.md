...
The backup path is shown in the UI under Settings → Automatic Backups (including the `backups/` subfolder) and is printed when the app is started from the command line. On desktop, **Change backup folder** in that same section lets you redirect automatic backups to any folder outside the app's data directory; existing backups in the old location are not moved and are not shown in the restore prompt.
...
- Location: `{userData}/backups/` by default, or a folder picked via **Change backup folder** in Settings → Automatic Backups.
...
**Automatic backups:** When enabled in settings, the app creates a backup on a schedule (default: every 5 minutes). The interval is not configurable in the UI. Storage location depends on platform:

| Platform               | Where automatic backups are stored                                                                                                                                                           |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Electron (desktop)** | `{userData}/backups/` by default, or a folder picked via **Change backup folder**, as timestamped JSON files (`YYYY-MM-DD_HHmmss.json`)                                                      |
...
On desktop, Settings → Automatic Backups shows the last successful backup time as of opening Settings. If a drive disconnects or the folder becomes unwritable, failed writes leave this timestamp unchanged. A custom backup folder that has gone missing is never re-created, so backups cannot silently end up on the wrong disk; they resume once the folder is back. The exception is a folder that is itself a mount point: it still looks present when unmounted, so backups then go to the empty directory underneath — prefer a subfolder of the mount point over the mount point itself. Legacy `YYYY-MM-DD.json` backups remain available for restore in the default folder only and are excluded from automatic cleanup.

Use a separate backup subfolder for each device in a shared cloud folder. Devices sharing one folder can overwrite backups made in the same second, prune each other's backups, and offer another device's snapshot for restore without identifying its origin. Cloud conflict copies are excluded from discovery and cleanup and require manual removal. Backup writes use a temporary file followed by a rename so the final JSON file is published only after the write completes.
