...
- On a touch screen, swipe a task left to open its menu and choose **Select several tasks…**. Every task then shows a ring instead of the done box: tap a task to select or deselect it, and use the bar's ✕, the back button or `Escape` to leave selection mode. Swiping, dragging and title editing are paused while selecting.
