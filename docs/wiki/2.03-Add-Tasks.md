...
- On a touch screen, swipe a task left to open its menu and choose **Select several tasks…**. Every task then shows a ring instead of the done box: tap a task to select or deselect it, and use the bar's ✕ or the back button to leave selection mode. Swiping, dragging and title editing are paused while selecting.

Selected tasks get a tinted background with an accent stripe on the left. There is deliberately no checkmark: a checkmark means _done_.

**Act**

A bar at the bottom of the list shows how many tasks are selected. **Actions** opens a menu with everything that can be applied to all of them: schedule, unschedule, add to or remove from Today, deadline, mark done or not done, move to backlog or back, estimate, tags, move to another project, delete. Right-clicking a selected task, or pressing `Q`, opens the same menu. The usual task shortcuts (`D`, `Backspace`, `S`, `Shift`+`S`, `Shift`+`T`, `U`, `Shift`+`B`) apply to the whole selection while it exists.
