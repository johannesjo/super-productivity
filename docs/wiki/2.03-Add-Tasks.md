...
- With a task focused, `Shift`+`ArrowDown` / `Shift`+`ArrowUp` extend the selection by keyboard.
...
- Deleting more than one task always asks for confirmation, because there is no undo for a bulk delete yet.
