...
The app supports **12 issue-style integrations** (the exact list may vary by version): **Jira**, **GitHub**, **GitLab**, **Gitea**, **OpenProject**, **CalDAV**, **Calendar (iCal)**, **Redmine**, **Trello**, **Linear**, **ClickUp**, and **Plane**. Each has a display name and icon in the Issue Panel. Configuration is per-provider and often per-project (e.g. which project new issues are added to).
