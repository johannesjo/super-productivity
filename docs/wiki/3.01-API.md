...
**Deadlines:** `deadlineDay` accepts a `YYYY-MM-DD` date string and `deadlineWithTime` accepts a Unix timestamp in milliseconds. They are mutually exclusive; setting one clears the other and applies the same automatic Today planning behavior as the UI. `deadlineRemindAt` is an optional reminder timestamp and requires a deadline. Supplying both deadline fields as non-null in one request returns 400 `INVALID_INPUT`.
