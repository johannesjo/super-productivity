...
**Deadlines:** `deadlineDay` accepts a valid calendar `YYYY-MM-DD` date. `deadlineWithTime` and `deadlineRemindAt` accept positive Unix timestamps in milliseconds. The deadline types are mutually exclusive; changing a deadline clears its existing `deadlineRemindAt` unless a new reminder is supplied in the same request. Re-sending the current deadline value, or nulling only its inactive field, is a no-op. A task with an existing deadline can update `deadlineRemindAt` alone. A deadline write applies the same automatic Today planning behavior as the UI, which can rewrite `dueDay` and clear `dueWithTime` plus `remindAt`. Supplying both deadline fields as non-null in one request returns 400 `INVALID_INPUT`.
