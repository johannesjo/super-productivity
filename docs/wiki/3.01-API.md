...
`Authorization: Bearer <token>`. Public registration and login flows do not
require an existing token. Invalid or missing credentials on a protected
endpoint return 401.

Token lifetime, claims, invalidation, and `JWT_SECRET` validation are defined in
`packages/super-sync-server/src/auth.ts`. Treat that file and the Sync Server
deployment documentation as the source of truth rather than copying those
security-sensitive values here.
