...
**Data — Recurring tasks:**

- `addTaskRepeatCfg(taskId, cfg?)` (daily by default), `updateTaskRepeatCfg(taskRepeatCfgId, updates)`, `deleteTaskRepeatCfg(taskRepeatCfgId)`. Read via `getAppState().taskRepeatCfgs`. `deleteTaskRepeatCfg` needs the `deleteTaskRepeatCfg` permission; the tasks the config created stay, detached.
