...
- `addTask(taskData)`, `updateTask(taskId, updates)`, `deleteTask(taskId)` — Create/update/delete. `parentId` must reference a top-level task; passing a subtask's id is rejected, since the model is only two levels deep.
