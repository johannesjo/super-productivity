...
> app.plane.so/acme-inc/projects/**1f2e3d4c-5b6a-4789-9abc-def012345678**/

Do not use the **Project ID** field in Plane's project settings. That one holds the short work-item
prefix (`ACME`), which is a different value.
...
Automatic backlog import stays off until you assign a **default project** and enable **auto
import** in the provider's Advanced Config — the same as for every provider (see
[Set a Default Project for an Integration](#set-a-default-project-for-an-integration)).
Searching and adding single work items from the Issue Provider Panel work without it.

**Plane is not available in the web app.** Its API does not send the CORS headers a browser
requires, so there the connection test fails and nothing imports. The desktop app and the
Android app work normally (both verified); iOS is expected to behave the same — requests go
through the native HTTP layer, not the browser — but has not been tested.
