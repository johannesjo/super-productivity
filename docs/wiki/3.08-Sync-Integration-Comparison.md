...
- **Connected devices:** Sync settings → Advanced → **Show Connected Devices** lists every device that has synced with the account recently, with its platform, client ID and last sync time. The list is read-only. Access to a SuperSync account is granted per account rather than per device — a single long-lived token, not one per device — so removing an entry would not stop that device from syncing, and no per-device revocation is offered. Devices that stop syncing drop off the list automatically after the server's retention period.
