...
- **Issue import:** Work items from one project of a Plane workspace, on Plane Cloud or a self-hosted instance.
- **Status transitions:** A task is marked done automatically when its Plane work item reaches the `completed` or `cancelled` state group. Pull-only — Super Productivity never writes back to Plane.
- **Worklog / Comments / Subtasks / Attachments:** Not supported.
- **Filtering:** Workspace slug plus project UUID; optional host for self-hosted instances.
- **Auto-import:** All open work items in the project, up to 2000 work items scanned per refresh. As with every provider, automatic import requires a default project and the auto-import toggle in the provider's Advanced Config; searching and importing single items works without them.
- **Due dates:** A work item's target date becomes the task's due day. Plane stores it as a calendar date with no time, so it lands on the same day regardless of your timezone.

Plane's public API has no state filter — the endpoint that accepts one belongs to Plane
Cloud — so the integration reads the project and decides which items are open on the
client. That keeps self-hosted instances on the same code path as Plane Cloud, at the
cost of one request per 100 work items.

**Not available in the web app.** Plane's API sends no `Access-Control-Allow-Origin` header,
so a browser blocks every request before it is sent; the connection test fails and nothing
imports. The desktop app and the Android app are unaffected (both verified) — they do not go
through the browser's CORS check; iOS is expected to behave the same but has not been tested.
Nothing in Super Productivity can change this; it needs Plane to publish CORS headers.
See [[3.05-Web-App-vs-Desktop]].
