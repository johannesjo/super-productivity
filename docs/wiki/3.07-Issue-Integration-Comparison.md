...
**Desktop app only.** Plane's API sends no `Access-Control-Allow-Origin` header, so a
browser blocks every request to it before it is sent. The desktop app is unaffected, but
in the web app the connection test fails and no work items are imported. Nothing in
Super Productivity can change this — it needs Plane to publish CORS headers.
