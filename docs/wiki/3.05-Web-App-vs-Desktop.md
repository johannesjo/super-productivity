...
The **Plane** issue provider is affected the same way: `api.plane.so` sends no `Access-Control-Allow-Origin` header, so in the web app the connection test fails and no work items import. It works in the desktop and mobile apps, which do not go through the browser's CORS check. See [[3.07-Issue-Integration-Comparison]] for the provider's other limits.
