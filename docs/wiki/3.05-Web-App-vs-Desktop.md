...
The **Plane** issue provider is affected the same way: `api.plane.so` sends no `Access-Control-Allow-Origin` header, so in the web app the connection test fails and no work items import. It works in the desktop app and the Android app (both verified), which do not go through the browser's CORS check; iOS is expected to behave the same but has not been tested. See [[3.07-Issue-Integration-Comparison]] for the provider's other limits.
