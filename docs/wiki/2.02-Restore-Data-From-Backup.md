...
Super Productivity also refuses this outright when a device has never completed a sync and holds no data of its own, so it cannot be run by accident from a blank device. If you see that refusal, the server copy is untouched.
