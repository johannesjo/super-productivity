...
- **Global (system-wide):** `globalShowHide`, `globalToggleTaskStart`, `globalAddNote`, `globalAddTask`, `globalTaskQuickAdd`. Available only in the Electron/desktop build.
...
- Global shortcuts (`globalShowHide`, `globalToggleTaskStart`, `globalAddNote`, `globalAddTask`, `globalTaskQuickAdd`) work system-wide when the app is running.
- `globalTaskQuickAdd` toggles the Quick Add HUD: a compact, frameless add-task bar that appears over whatever you are doing, without bringing the main window forward. Pressing the key again closes it, as does `Escape` or a click outside. It is unbound by default. The task is created through the normal app pipeline, so short syntax, projects, tags and recurrence all behave as they do in the app.
...
| `superproductivity://quick-add`                   | `globalTaskQuickAdd`    | Open the Quick Add HUD over the current window, without focusing the main window. Unlike the shortcut this only opens; the HUD closes itself when it loses focus.                                                                                                                    |
