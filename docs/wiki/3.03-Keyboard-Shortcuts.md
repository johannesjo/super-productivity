...
The cheat sheet lists your current shortcuts, so it reflects any bindings you have changed in [[3.02-Settings-and-Preferences]]. The shortcut matches the `?` character itself, so it also works on layouts where `?` sits on a different key.
