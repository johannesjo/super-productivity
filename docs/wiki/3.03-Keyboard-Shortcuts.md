...
| `superproductivity://complete-task/<title>` | —                       | Complete the single unfinished top-level task matching the URL-encoded `<title>` (also accepts `?title=`). Surrounding whitespace is trimmed; an exact match wins, otherwise an unambiguous substring match. See [[3.01-API]] for the full contract. |
