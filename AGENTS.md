...
- **Hardening needs an observed instance:** before adding a guard — a lint-rule branch, a runtime assertion, a defensive check — grep this repo for a real occurrence of the shape it catches. Zero occurrences → record it as a known gap instead of guarding it. This generalises the sync section's "start from a reproducible problem" to any defensive change. Treat a review finding as a hypothesis to measure, not a work order. A generated allowlist (grandfathered offenders) must never grow to absorb a false positive the change itself introduced — fix the check, or scope a disable with a reason. Before trusting a check that passed, confirm it can fail → [`docs/hardening-earns-its-place.md`](docs/hardening-earns-its-place.md).
